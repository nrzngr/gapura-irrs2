    ...
    return (
        <div className="space-y-6 pb-24 stagger-children">
            ...
            <div className="card-solid p-0 overflow-hidden animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                <div className="p-6 border-b border-[var(--surface-4)] bg-emerald-50/50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <Building2 size={20} className="text-emerald-600" />
                        <div>
                            <h3 className="font-bold text-lg text-[var(--text-primary)]">Performa Regional</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[var(--surface-4)]">
                    {analytics?.stationData?.slice(0, 9).map((station) => {
                        const efficiency = Math.round((station.resolved / Math.max(station.total, 1)) * 100);
                        const isHighPerformer = efficiency >= 80;
                        
                        return (
                            <div 
                                key={station.station} 
                                className="p-6 bg-white hover:bg-[var(--surface-2)] transition-all cursor-pointer group"
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h4 className="font-bold text-xl text-[var(--text-primary)] group-hover:text-emerald-600 transition-colors">
                                            {station.station}
                                        </h4>
                                        <p className="text-xs text-[var(--text-muted)]">{station.total} laporan</p>
                                    </div>
                                    <div className={cn(
                                        "w-10 h-10 rounded-xl flex items-center justify-center transition-all",
                                        isHighPerformer ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600",
                                        "group-hover:scale-110 group-hover:rotate-3"
                                    )}>
                                        {isHighPerformer ? <ShieldCheck size={18} /> : <Target size={18} />}
                                    </div>
                                </div>
                                
                                <div className="space-y-2">
                                    <div className="flex justify-between text-xs">
                                        <span className="text-[var(--text-muted)]">Efisiensi</span>
                                        <span className={cn(
                                            "font-bold",
                                            isHighPerformer ? "text-emerald-600" : "text-amber-600"
                                        )}>
                                            {efficiency}%
                                        </span>
                                    </div>
