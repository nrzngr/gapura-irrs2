...
const DIVISION = { code: 'HC', name: 'Human Capital', color: '#8b5cf6' };
...
export default function HCReportsPage() {
    ...
    const fetchReports = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/admin/reports?target_division=HC');
    ...
    return (
        <div className="space-y-8 stagger-children pb-24">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 animate-fade-in-up">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <Users size={20} style={{ color: DIVISION.color }} />
                    ...
                    <p className="mt-1" style={{ color: 'var(--text-secondary)' }}>Kelola laporan Human Capital</p>
                </div>
            ...
            <div className="card-solid animate-fade-in-up" style={{ padding: 0, overflow: 'hidden', animationDelay: '150ms' }}>
                <div className="flex items-center justify-between" style={{ padding: 'var(--space-lg) var(--space-xl)', borderBottom: '1px solid var(--surface-4)' }}>
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 rounded-xl" style={{ background: `${DIVISION.color}20` }}><Users size={18} style={{ color: DIVISION.color }} /></div>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {filteredReports.map((r) => {
                                    ...
                                    return (
                                        <tr key={r.id} className="cursor-pointer transition-colors" style={{ borderBottom: '1px solid var(--surface-4)', borderLeft: `3px solid ${sev.color}` }} onClick={() => router.push(`/dashboard/hc/reports/${r.id}`)} onMouseEnter={(e) => e.currentTarget.style.background = 'var(--surface-3)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
            ...
            {selectedReport && <QuickModal report={selectedReport} onClose={() => setSelectedReport(null)} color={DIVISION.color} path={`/dashboard/hc/reports/${selectedReport.id}`} />}
        </div>
    );
}

function StatCard({ icon: Icon, value, label, color }: { icon: React.ComponentType<{ size?: number; style?: React.CSSProperties }>; value: number; label: string; color?: string }) {
...
function FilterSelect({ value, onChange, icon: Icon, options }: { value: string; onChange: (v: string) => void; icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }>; options: { value: string; label: string }[] }) {
