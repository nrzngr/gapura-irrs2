  ...
  const fetchAll = async () => {
    ...
    try {
      const [resA, resB] = await Promise.all([
        fetch('/api/ai/analyze-all?exclude_closed=true&division=OT'),
        fetch('/api/ai/root-cause/stats?division=OT'),
      ]);
      ...
      <section className="relative overflow-hidden bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-800">AI Reports · Divisi OT</h1>
              <p className="text-xs text-gray-500">Analisis otomatis: prioritas, pola risiko, dan rekomendasi</p>
            </div>
          </div>
          <button
            onClick={fetchAll}
