      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="h-16 px-4 sm:px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900 tracking-tight">Area Report</h1>
              <p className="text-xs text-gray-500">Detailed analysis by area performance & risk profile</p>
        ...
        <div className="px-4 sm:px-6 pb-4 border-b border-gray-100">
          <div className="flex flex-wrap gap-3">
            ...
            >
              ...
              {['Terminal 1', 'Terminal 2', 'Terminal 3', 'CGK', 'SUB'].map(b => (
                <option key={b} value={b}>{b}</option>
              ))}
              {filters.branch !== 'all' && !['Terminal 1', 'Terminal 2', 'Terminal 3', 'CGK', 'SUB'].includes(filters.branch) && (
                <option value={filters.branch}>{filters.branch}</option>
              )}
            </select>
            ...
            >
              ...
              {['Landside', 'Airside', 'Irregularity', 'Complaint', 'Compliment'].map(a => (
                <option key={a} value={a}>{a}</option>
              ))}
              {filters.area !== 'all' && !['Landside', 'Airside', 'Irregularity', 'Complaint', 'Compliment'].includes(filters.area) && (
                <option value={filters.area}>{filters.area}</option>
              )}
            </select>
          </div>
        </div>
