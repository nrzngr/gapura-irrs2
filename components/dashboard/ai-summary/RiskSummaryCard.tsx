    ...
    <div className={cn("space-y-4", className)}>
      ...
      {overview && (
        <motion.div
          ...
          className="grid grid-cols-2 sm:grid-cols-4 gap-2 p-3 rounded-xl bg-gradient-to-r from-slate-50/80 to-gray-50/80 border border-gray-100"
        >
          <div className="text-center">
            ...
            <p className="text-base font-black text-gray-800 tabular-nums">
              {overview.total_issues.toLocaleString("id-ID")}
            </p>
          </div>
          <div className="text-center">
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">
            ...
            <p
              className={cn(
                ...
                getRiskScoreColor(overview.avg_risk_score_airlines)
              )}
            >
              {overview.avg_risk_score_airlines.toFixed(1)}
            </p>
          </div>
          <div className="text-center">
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">
            ...
            <p
              className={cn(
                "text-base font-black tabular-nums",
                getRiskScoreColor(overview.avg_risk_score_branches)
              )}
            >
              {overview.avg_risk_score_branches.toFixed(1)}
            </p>
          </div>
          <div className="text-center">
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">
            ...
            <p
              className={cn(
                "text-base font-black tabular-nums",
                getRiskScoreColor(overview.avg_risk_score_hubs)
              )}
            >
              {overview.avg_risk_score_hubs.toFixed(1)}
            </p>
          </div>
        </motion.div>
      )}
      ...
      {issueSeverity && (
        <div className="space-y-2.5 p-3 rounded-xl bg-gray-50/60 border border-gray-100">
          <p className="text-[10px] font-bold text-gray-500 uppercase">
            Distribusi Severity Laporan
          </p>
          {SEVERITY_LEVELS.map((level) => {
            const count = issueSeverity[level] || 0;
            const pct =
              maxIssueSeverity > 0
                ? Math.min((count / maxIssueSeverity) * 100, 100)
                : 0;
            const style = LEVEL_STYLE[level];
            return (
